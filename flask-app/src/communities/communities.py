...
# unsubscribes this user from membership to a given community
@communities.route.route('/remove-membership/<int:user_id>', methods=['DELETE'])
def remove_membership(user_id):
    community = request.json

    if 'communityID' not in community:
    ...
    '''
    cursor = db.get_db().cursor()
    cursor.execute(query, (communityID, user_id))
    db.get_db().commit()
